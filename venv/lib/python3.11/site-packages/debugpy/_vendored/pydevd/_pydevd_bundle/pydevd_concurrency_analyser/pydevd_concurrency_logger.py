        ...
        if "self" in frame.f_locals:
            ...
            if isinstance(self_obj, asyncio.Task):
                ...
                if method_name == "set_result":
                    task_id = id(self_obj)
                    title = self.task_mgr.get(str(task_id))
                    send_concurrency_message("asyncio_event", event_time, title, title, "thread", "stop", frame.f_code.co_filename,
                ...
                if method_name == "__init__":
                    task_id = id(self_obj)
                    title = self.task_mgr.get(str(task_id))
                    send_concurrency_message("asyncio_event", event_time, title, title, "thread", "start", frame.f_code.co_filename,
                                 frame.f_lineno, frame)
                ...
                if method_name in ("acquire", "release"):
                    ...
                    if method_name == "acquire":
                        if not self_obj._waiters and not self_obj.locked():
                            send_concurrency_message("asyncio_event", event_time, title, title, "lock",
